...
from isaac_utils.rotations import get_euler_xyz_in_tensor, quat_from_angle_axis, quat_mul_norm
...
class LeggedRobotBase(BaseTask):
    ...
    def _init_buffers(self):
        ...
        if hasattr(self, "end_effector_index"):
            ...
            self.end_effector_rot_gravity = quat_rotate_inverse(self.end_effector_rot, self.gravity_vec)
    ...
    def _pre_physics_step(self, actions):
        ...
        self.actions = torch.clip(self.actions, -clip_action_limit, clip_action_limit).to(self.device)
        ...
        if hasattr(self, "end_effector_index"):
            self.pre_end_effector_vel[:] = self.simulator._rigid_body_vel[:, self.end_effector_index, :]
            self.pre_end_effector_ang_vel[:] = self.simulator._rigid_body_ang_vel[:, self.end_effector_index, :]
    ...
    def _post_physics_step(self):
        ...
        if len(refresh_env_ids) > 0:
            self.simulator.set_actor_root_state_tensor(refresh_env_ids, self.simulator.robot_root_states)
        ...
        pitch_angle = torch.tensor(np.pi/2., device=self.device).repeat(self.num_envs)
        pitch_axis = torch.tensor([0., 1., 0.], device=self.device).repeat(self.num_envs, 1)
        pitch_quat = quat_from_angle_axis(pitch_angle, pitch_axis, w_last=True)
        
        # Create yaw rotation from base
        yaw_axis = torch.tensor([0., 0., 1.], device=self.device).repeat(self.num_envs, 1)
    ...
    def _update_reset_buf(self):
        ...
        if self.config.termination.terminate_by_low_height:
            ...
            self.reset_buf |= torch.any(robot_height < self.config.termination_scales.termination_min_base_height, dim=1)
    ...
    def _reset_buffers_callback(self, env_ids, target_buf=None):
        if target_buf is not None:
            ...
            if hasattr(self, "end_effector_index"):
                ...
                self.pre_end_effector_vel[env_ids] = target_buf["pre_end_effector_vel"].to(self.pre_end_effector_vel.dtype)
                self.pre_end_effector_ang_vel[env_ids] = target_buf["pre_end_effector_ang_vel"].to(self.pre_end_effector_ang_vel.dtype)
            ...
            if hasattr(self, "end_effector_index"):
                ...
                self.pre_end_effector_vel[env_ids] = 0.
                self.pre_end_effector_ang_vel[env_ids] = 0.
    ...
    def _compute_reward(self):
        ...
        if self.use_reward_penalty_curriculum:
            self.log_dict["reward_penalty_scale"] = torch.tensor(self.reward_penalty_scale, dtype=torch.float, device=self.device)
    ...
    def _compute_torques(self, actions):
        ...
        if self.config.robot.control.get("rescale_action_by_dof_limit", False):
            actions_scaled = actions * self.dof_scales
        else:
    ...
    def _draw_debug_vis(self):
        ...
        for env_id in draw_env_ids:
            ...
            for push_vel, push_pos, push_mag, push_color, push_line_width in zip(push_vel_list, push_pos_list, push_mag_list, push_color_schems, push_line_widths):
                for _ in range(200):
                    self.simulator.draw_line(Point(push_pos +torch.rand(3, device=self.device) * push_line_width),
                                        Point(push_pos + push_vel * push_mag),
                                        Point(push_color), env_id)
    ...
    def _reset_dofs(self, env_ids, target_state=None):
        ...
        if target_state is not None:
            ...
            # self.simulator.dof_pos[env_ids] = self.default_dof_pos; print("debugging")
            # import ipdb; ipdb.set_trace()
    ...
    def _reset_root_states(self, env_ids, target_root_states=None):
        ...
        if target_root_states is not None:
            ...
            # self.simulator.robot_root_states[env_ids, 7:13] = torch.zeros((len(env_ids), 6), device=str(self.device)); print("debugging") # [7:10]: lin vel, [10:13]: ang vel
    ...
    def _get_obs_history_cenet(self,):
        assert "history_cenet" in self.config.obs.obs_auxiliary.keys()
        history_config = self.config.obs.obs_auxiliary['history_cenet']
        history_key_list = history_config.keys()
        history_tensors = []
        for key in sorted(history_config.keys()):
            history_length = history_config[key]
    ...
    def _load_z_model(self,):
        ...
        config_path = Path(check_points_path[0]).parent / "config.yaml"
        checkpoint = torch.load(check_points_path[0], map_location=self.device, weights_only=False)
        with open(config_path) as file:
                self.z_config = OmegaConf.load(file)
                pre_process_config(self.z_config)
        
        if self.z_config.algo.config.use_new_actor_critic :
            policy = instantiate(self.z_config.algo.config.actor, env_config=self.z_config.env.config, algo_config=self.z_config.algo.config, _recursive_=False).to(self.device)
            policy.eval()
            self.z_actor = policy
    ...
    def _compute_z_prior(self):
        ...
        for obs_key, obs_config in self.z_config.obs.obs_dict.items():
            if obs_key == 'proprioception_obs':
                ...
                parse_observation(self, obs_config, teacher_obs_dict_raw[obs_key], self.z_config.obs.obs_scales, self.z_config.obs.noise_scales, noise_curriculum_value)
                
        for key in teacher_obs_dict_raw['proprioception_obs'].keys(): # prefer to use the same noised obs for z_prior as buf_dict_raw
            if key in self.obs_buf_dict_raw['actor_obs']:
                teacher_obs_dict_raw['proprioception_obs'][key] = self.obs_buf_dict_raw['actor_obs'][key]
        ...
        for obs_key, obs_config in self.z_config.obs.obs_dict.items():
            if obs_key == 'proprioception_obs':
                obs_keys = sorted(obs_config)
                # print("obs_keys", obs_keys)            
                self.z_obs_dict[obs_key] = torch.cat([teacher_obs_dict_raw[obs_key][key] for key in obs_keys], dim=-1)
        
        with torch.no_grad():
            self.prior_mu, self.prior_sigma  = self.z_actor.eval_prior(None, obs_dict=self.z_obs_dict)
