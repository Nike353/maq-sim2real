...
class Terrain:
    ...
    def single_slope_terrain_func(self, terrain, difficulty):
        height = 0.05 * difficulty  # Set hill height based on difficulty
        platform_size = 4  # Width of the peak region
        height = int(height / terrain.vertical_scale)
        platform_size = int(platform_size / terrain.horizontal_scale / 2) 
        print('height:', height, 'platform_size:', platform_size)
        print(terrain.vertical_scale)
        print(terrain.horizontal_scale)
        # import ipdb; ipdb.set_trace()

        x_mid = int(terrain.length / 2)  # Midpoint of terrain

        # Create an upward slope (first half)
        for x in range(0, x_mid - platform_size):
            print(x_mid - platform_size)
            slope_factor = x / x_mid  # Normalized slope factor from 0 to 1
            terrain.height_field_raw[x, :] = slope_factor * height  

        # Create a downward slope (second half)
        for x in range(x_mid + platform_size, terrain.length):
            slope_factor = (terrain.length - x) / x_mid
            terrain.height_field_raw[x, :] = slope_factor * height
