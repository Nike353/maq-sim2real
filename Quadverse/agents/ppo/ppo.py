...
import humanoidverse.agents.callbacks
...
class PPO(BaseAlgo):
    ...
    def _init_config(self):
        ...
        self.save_latest_interval = self.config.get("save_latest_interval", 50)
    ...
    def load(self, ckpt_path):
        ...
        if ckpt_path is not None:
            ...
            if "policy_state_dict" in loaded_dict:
                self.actor.load_state_dict(loaded_dict["policy_state_dict"])
            else:
                self.actor.load_state_dict(loaded_dict["actor_model_state_dict"])
            if "value_state_dict" in loaded_dict:
                self.critic.load_state_dict(loaded_dict["value_state_dict"])
            else:
            ...
            if self.load_optimizer:
                if "actor_optimizer_state_dict" in loaded_dict:
                    ...
                    logger.info(f"No optimizer state dict found in checkpoint")
    ...
    def learn(self):
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            log_dict = {
                ...
                'curriculum_infos': self.curriculum_infos,
            ...
            for i in range(self.num_steps_per_env):
                ...
                if self.log_dir is not None:
                    ...
                    if 'curriculum' in infos:
                        self.curriculum_infos.append(infos['curriculum'])
        ...
        if log_dict['ep_infos']:
            ...
            for key in log_dict['ep_infos'][0]:
                ...
                if wandb.run is not None:
                    wandb.log({'Episode/' + key: value}, step=log_dict['it'])
                ep_string += f"""{f'Mean episode {key}:':>{pad}} {value:.4f}\n"""

        if log_dict['curriculum_infos']:
            for key in log_dict['curriculum_infos'][0]:
                if key == 'phase':
                    value = log_dict['curriculum_infos'][-1][key] # NOTE: lingyun - use the last phase for the current iteration
                else:
                    infotensor = torch.tensor([], device=self.device)
                    for curriculum_info in log_dict['curriculum_infos']:
                        if not isinstance(curriculum_info[key], torch.Tensor):
                            curriculum_info[key] = torch.Tensor([curriculum_info[key]])
                        infotensor = torch.cat((infotensor, curriculum_info[key].to(self.device)))
                    value = torch.mean(infotensor)
                self.writer.add_scalar('Curriculum/' + key, value, log_dict['it'])
                ep_string += f"""{f'Mean episode {key}:':>{pad}} {value:.4f}\n"""
    ...
    def _logging_to_writer(self, log_dict, train_log_dict, env_log_dict):
        # Logging Loss Dict
        if wandb.run is not None:
            wandb.log({f"Loss/{k}": v for k, v in log_dict['loss_dict'].items()}, step=log_dict['it'])
        
            wandb.log({
                'Loss/actor_learning_rate': self.actor_learning_rate,
                'Loss/critic_learning_rate': self.critic_learning_rate,
                'Policy/mean_noise_std': train_log_dict['mean_std'],
                'Perf/total_fps': train_log_dict['fps'],
                'Perf/collection time': log_dict['collection_time'],
                'Perf/learning_time': log_dict['learn_time']
            }, step=log_dict['it'])
            
            if len(log_dict['rewbuffer']) > 0:
                wandb.log({
                    'Train/mean_reward': statistics.mean(log_dict['rewbuffer']),
                    'Train/mean_episode_length': statistics.mean(log_dict['lenbuffer']),
                    'Train/mean_reward/time': statistics.mean(log_dict['rewbuffer']),
                    'Train/mean_episode_length/time': statistics.mean(log_dict['lenbuffer'])
                }, step=log_dict['it'])
            if len(env_log_dict) > 0:
                wandb.log(env_log_dict, step=log_dict['it'])
    ...
    def evaluate_policy(self):
        with torch.inference_mode():
            ...
            actor_state.update({"obs": obs_dict, "actions": init_actions,"step": 0})
