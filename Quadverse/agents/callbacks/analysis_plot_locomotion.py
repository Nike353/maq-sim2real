...
class AnalysisPlotLocomotion(RL_EvalCallback):
    ...
    def __init__(self, config, training_loop: PPO):
        ...
        policy_checkpoint_dir = self.env.config.ckpt_dir
        sim_type = self.env.config.simulator['_target_'].split('.')[-1]
        self.log_error_path_dir = os.path.join(policy_checkpoint_dir, "log_error")
        os.makedirs(self.log_error_path_dir, exist_ok=True)
        self.log_error_path = os.path.join(self.log_error_path_dir, f"{sim_type}.npz")
    ...
    def _generate_commands(self, actor_state):
        ## generate commands for locomtoion , just forward, but ramp signal, ramp up to 0.6 for 0.25*max_epsiode then keep at 0.6 for 0.5*max_epsiode then ramp down to 0 for 0.25*max_epsiode
        # max_episode_length = self.env.max_episode_length
        # step = actor_state["step"]%max_episode_length
        # if self.env.is_evaluating:
        #     if step < max_episode_length * 0.25:
        #         self.env.commands[:, 0] = min(step/(max_episode_length * 0.25),0.6)
        #     elif step < max_episode_length * 0.75:
        #         self.env.commands[:, 0] = 0.6
        #     else:
        #         self.env.commands[:, 0] = max(0.6 - (step - max_episode_length * 0.75)/(max_episode_length * 0.25),0)
        ...
        max_episode_length = self.env.max_episode_length
        step = actor_state["step"]%max_episode_length
        if self.env.is_evaluating and step ==0:
            self.env.commands[:, 0] = torch.rand(self.num_envs, 1) * 2 -1
            self.env.commands[:, 1] = torch.rand(self.num_envs, 1) * 2 -1
        self.env.commands[:, 2] = 0
        self.env.commands[:, 3] = 0
        return actor_state
    ...
    def on_pre_eval_env_step(self, actor_state):
        ...
        # print(actor_state["step"], self.env.commands[:, 0])
            ...
            self.logger.log_states(
                {
                    ...
                    'command_x': self.env.commands[:, 0].mean().item(),
                    'command_y': self.env.commands[:, 1].mean().item(),
                    'command_yaw': self.env.commands[:, 2].mean().item(),
    ...
    def on_post_eval_env_step(self, actor_state):
        ...
        step = actor_state["step"]%self.env.max_episode_length
        if step == 0:
            print(len(self.logger.tracking_errors))
            self.logger.tracking_errors.append(np.mean(self.logger.current_epsiode_errors))
            self.logger.current_epsiode_errors = []
        if len(self.logger.tracking_errors) >=50:
            print("collected 30 episodes.Final_mean_tracking_error: ", np.mean(self.logger.tracking_errors))
            np.savez(self.log_error_path, tracking_errors=self.logger.tracking_errors,mean_tracking_error=np.mean(self.logger.tracking_errors))
            exit()
        return actor_state
...
class WebLogger:
    ...
    def log_states(self, dict):
        ...
        #compute tracking error for current timestep
        vel_x = dict["base_vel_x"]
        vel_y = dict["base_vel_y"]
        
        command_x = dict["command_x"]
        command_y = dict["command_y"]
        
        error = np.linalg.norm([vel_x - command_x, vel_y - command_y])
        self.current_epsiode_errors.append(error)
    ...
    def _update_plot(self):
        ...
        for i in range(forces[0].shape[0]):
            ...
            print(f"Force {i} : {len([force[i] for force in forces])}")

        # Command velocities
        add_trace(time, log["command_x"], BLUE, 2, 2, "Command x")
        print(f"Command x : {len(log['command_x'])}")
