...
class AnalysisPlotMotionTracking(RL_EvalCallback):
    ...
    def reset_buffers(self):
        self.obs_buf = [deque(maxlen=1000) for _ in range(self.num_envs)]
        self.critic_obs_buf = [deque(maxlen=1000) for _ in range(self.num_envs)]
        self.act_buf = [deque(maxlen=1000) for _ in range(self.num_envs)]
            ...
            if self.curr_stpes >= curr_max or self.terminate_state.sum() == self.env.num_envs:
                ...
                if (self.env.start_idx + self.env.num_envs >= self.env._motion_lib._num_unique_motions):
                    ...
                    if self.only_print_second_iter:
                        ...
                        metrics_print['success_rate'] = self.success_rate
                        actor_state['metrics_all'] = metrics_all_print
                        actor_state['metrics_success'] = metrics_print
