...
class PPODecouple(PPO):
    ...
    def _logging_to_writer(self, log_dict, train_log_dict, env_log_dict):
        ...
        if self.env.config.rewards.get("upper_body_motion_scale_curriculum", False):
            wandb.log({'Env/action_scale_upper_body': self.env.action_scale_upper_body.mean().item()}, step=log_dict['it'])
        if self.env.config.rewards.get("command_height_scale_curriculum", False):
            
            wandb.log({'Env/command_height_scale': self.env.command_height_scale.mean().item()}, step=log_dict['it'])
