...
class IsaacGym(BaseSimulator):
    ...
    def _create_trimesh(self):
        ...
        self.terrain.vertices = self.terrain.vertices.astype(np.float32)
    ...
    def _build_objects(self, env_id, env_ptr):
        # Create balls
        self.ball_radius = 0.005 # Small ball radius
        ball_opts = gymapi.AssetOptions()
        self.ball_asset = self.gym.create_sphere(self.sim, self.ball_radius, ball_opts)
        self.ball_ids = []
        self.ball_color = gymapi.Vec3(0.0, 0.0, 0.5)  # Orange
        for i in range(1000):  # 100 small balls

            # print("self.envs._rigid_body_pos", self._rigid_body_pos[:, self.end_effector_index])
            z = 0.88+ 0.01* (i)  # Slightly above the tote
            x = 0.25 + 0.02 * (i % 10)
            y = -0.3 + 0.0005 * (i / 10)
            # import ipdb; ipdb.set_trace()
            ...
            ball_id = self.gym.create_actor(env_ptr, self.ball_asset, ball_pose, f"ball_{i}", env_id, 0, 0)
            self.ball_ids.append(ball_id)

            # Set ball color
            self.gym.set_rigid_body_color(env_ptr, ball_id, 0, gymapi.MESH_VISUAL, self.ball_color)

        print(f"Loaded {len(self.ball_ids)} balls into the environment.")
    ...
    def create_envs(self, num_envs, env_origins, base_init_state):
        ...
        self.envs = []
        self.robot_handles = []
        self.base_mass_scale = torch.ones(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.env_config.domain_rand.randomize_base_com:
            ...
            try:
                ...
                torso_index = self._body_list.index("base") # for fixed upper URDF we only have pelvis link
                if torso_index == -1:
                    torso_index = self._body_list.index("base")
        ...
        if self.env_config.domain_rand.randomize_link_mass:
            ...
            for i, body_name in enumerate(self.env_config.robot.randomize_link_body_names):
                ...
                if body_name == "base":
                    self.base_mass_scale[env_id] *= mass_scale
    ...
    def setup_viewer(self):
        ...
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_I, "waist_yaw_up"
        )
        self.gym.subscribe_viewer_keyboard_event(
            self.viewer, gymapi.KEY_K, "waist_yaw_down"
        )
