...
class RobotInterface {
 public:
  RobotInterface(uint8_t level) :  udp(level, 8080, "192.168.123.161", 8082) {
    // InitEnvironment();
    if (level == 0xee) {
      udp.InitCmdData(high_cmd);
      cout << "High level command initialized" << endl;
    } else {
      udp.InitCmdData(low_cmd);
      cout << "Low level command initialized" << endl;
    }
  }
...
void RobotInterface::SendHighCommand(float forwardSpeed, float sideSpeed, float rotateSpeed,
    ...
    high_cmd.reserve = 0;
    high_cmd.footRaiseHeight = 0.1;
    high_cmd.bodyHeight = bodyHeight;
    cout << "Sending high command" << endl;
