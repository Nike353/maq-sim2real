...
class LocomotionPolicy(RLPolicy):
    def __init__(self, 
        ...
        self.mocap_lin_vel = None
        self.mocap_pos = None
        self.mocap_quat = None
        self.mocap_euler = None
        if self.use_mocap:
            # Subscribers
    ...
    def prepare_obs_for_rl(self, robot_state_data):
        ...
        # import ipdb; ipdb.set_trace()   
        # print(base_ang_vel)
        if self.use_clock_input: # hardcode for Ziyan genesis
            # obs = np.concatenate([self.last_policy_action[:, :10], 
            #                         base_ang_vel*0.25, 
            #                         self.ang_vel_command, 
            #                         self.lin_vel_command, 
            #                         dof_pos_minus_default[:, :10], 
            #                         dof_vel[:, :10]*0.05,
            #                         projected_gravity,
            #                         clock_input()
            #                         ], axis=1)
