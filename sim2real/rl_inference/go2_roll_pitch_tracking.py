...
class LocomotionPolicy(RLPolicy):
    ...
    def prepare_obs_for_rl(self, robot_state_data):
        ...
        if self.epi_len<150 :
            self.rp_command[0, 1] = 0
            self.rp_command[0, 0] = 0
            ...
            R_ref = R.from_euler('y', self.rp_command[0, 0])

        elif int(self.epi_len/150)%2 == 1:
            self.rp_command[0, 1] = 0
            self.rp_command[0, 0] = self.pitch_ref(self.epi_len)
            obs = np.concatenate([self.last_policy_action[:, :12], 
                                    base_ang_vel*0.25,
                                    self.rp_command,
                                    dof_pos_minus_default[:, :12],
            ...
            self.rp_command[0, 0] = 0
            self.rp_command[0, 1] = self.pitch_ref(self.epi_len)
            obs = np.concatenate([self.last_policy_action[:, :12], 
                                    base_ang_vel*0.25,
                                    self.rp_command,
                                    dof_pos_minus_default[:, :12],
            ...
            R_ref = R.from_euler('x', self.rp_command[0, 1])
        ...
        ref_msg.twist.twist.linear.x = 0.0
        ref_msg.twist.twist.linear.y = 0.0
        ref_msg.twist.twist.angular.z = 0.0
