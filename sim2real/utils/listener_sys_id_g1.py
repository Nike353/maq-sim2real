...
class DataLogger(Node):
    def __init__(self, save_path='data_log.npz', log_interval=1.0, buffer_rate=200.0, buffer_size=200*300, ignore_ref=False, robot_type='go2'):
        ...
        self.sub_state = ChannelSubscriber("rt/lowstate", LowState_hg)
        self.sub_state.Init(self.LowStateHandler, 10)
                
        self.sub_cmd = ChannelSubscriber("rt/lowcmd", LowCmd_hg)
    ...
    def LowStateHandler(self, msg):
        ...
        self.latest_low_state = {
            'timestamp': timestamp,
            'motor_positions': np.array([motor.q for motor in msg.motor_state]),
            'motor_velocities': np.array([motor.dq for motor in msg.motor_state]),
            'motor_torques': np.array([motor.tau_est for motor in msg.motor_state]),
    ...
    def LowCmdHandler(self, msg):
        ...
        self.latest_low_cmd = {
            'timestamp': timestamp,
            'motor_commands': {
                "q": np.array([motor_cmd.q for motor_cmd in msg.motor_cmd]),
                "dq": np.array([motor_cmd.dq for motor_cmd in msg.motor_cmd]),
                "kp": np.array([motor_cmd.kp for motor_cmd in msg.motor_cmd]),
                "kd": np.array([motor_cmd.kd for motor_cmd in msg.motor_cmd]),
                "tau": np.array([motor_cmd.tau for motor_cmd in msg.motor_cmd])
            }
        }
