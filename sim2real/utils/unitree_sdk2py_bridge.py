...
class UnitreeSdk2Bridge:
    ...
    def __init__(self, mj_model, mj_data, robot_config):
        ...
        self.base_state = Pose_default()
        self.base_state_puber = ChannelPublisher("rt/base_state", Pose_go)
        ...
        if self.mj_data != None:
            if self.use_sensor:
                ...
                for i in range(self.num_motor):
                    ...
                    if self.free_base:
                        self.low_state.motor_state[i].tau_est = self.mj_data.actuator_force[6+i]
                    else: self.low_state.motor_state[i].tau_est = self.mj_data.actuator_force[i]
                    # TODO: temperature (Default: human body temperature :))
                    # self.low_state.temperature[0] = 37.5
                    # self.low_state.temperature[1] = 37.5
            ...
            if self.use_sensor and self.have_frame_sensor_:
                ...
                # acceleration: x, y, z
                self.low_state.imu_state.accelerometer[0] = self.mj_data.qacc[0]
                self.low_state.imu_state.accelerometer[1] = self.mj_data.qacc[1]
                self.low_state.imu_state.accelerometer[2] = self.mj_data.qacc[2]
            self.low_state.tick = int(self.mj_data.time*1e3)
            self.low_state_puber.Write(self.low_state)
    def PublishBaseState(self):
        if self.mj_data is None:
            return
        
        self.base_state.position.x = self.mj_data.qpos[0]
        self.base_state.position.y = self.mj_data.qpos[1]
        self.base_state.position.z = self.mj_data.qpos[2]
        self.base_state.orientation.w = self.mj_data.qpos[3]
        self.base_state.orientation.x = self.mj_data.qpos[4]
        self.base_state.orientation.y = self.mj_data.qpos[5]
        self.base_state.orientation.z = self.mj_data.qpos[6]
        self.base_state_puber.Write(self.base_state)

        ##TODO: change this to handle mocap data later
